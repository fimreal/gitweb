...
1. **文件树浮窗停靠功能完善**
   ...
   - 问题是用户感知不到/触发不够直观：给按钮加 hover/title 提示，让停靠菜单视觉更明显，停靠后样式更精致。
2. **浮岛边缘吸附动画优化**
   - 当前 `SNAP_DIST = 44` 且按元素中心计算，导致未到边缘就吸附；拖动时 `.dragging { transition: none }` 让 scale 突变。
   - 改为：真正接近边缘（元素边缘距视口边缘 ≤ 阈值）才触发吸附；吸附时平滑过渡，拖动过程中实时跟随不跳变。
3. **设置按钮切换逻辑改为"显示当前状态"**
   - 主题/语言/隐藏文件/显示行号 四个二态开关：按钮文案和图标显示**当前状态**，点击后切换到另一状态。
   - 例如：当前暗色 → 按钮显示"暗色"+月亮图标；点击后切亮色 → 显示"亮色"+太阳图标。
4. **i18n 补全**
   - 文件树标题"文件目录"、首页链接"← 首页"、气泡提示、加载/刷新提示等切换语言时跟随。
5. **全面代码 review**
   - 检查事件绑定重复、闭包变量、CSS 优先级、移动端 touch 等。修复明显问题。暂不引入新 feature。
