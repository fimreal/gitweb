...
1. **文件树浮窗停靠功能完善**
   - 停靠交互改为**双击标题栏**→默认停靠左侧；停靠态双击→退出停靠。
   - 停靠后标题栏出现**向下箭头按钮**（#dock-switch-btn），点击弹出停靠侧菜单（左/右/底）。
   - 停靠面板支持**拖拽边缘调整宽度/高度**（8px 检测区，CSS 自定义属性 --dock-w/--dock-h）。
   - 尺寸持久化到 localStorage，页面刷新后恢复停靠态与尺寸。
   - 拖标题栏可撕下来变回浮窗（capture 阶段拦截，与 resize 不冲突）。
   - 停靠态标题栏紧凑 padding，避免标题文字被挤压。
2. **浮岛边缘吸附动画优化**
   - 距离计算从元素中心改为元素**边缘**，SNAP_DIST 44→12，真正贴边才吸附。
   - 拖动中不加 snapped class（避免 scale/圆角突变）；拖动开始时移除吸附态。
   - 松手时才判断吸附，用缓动过渡到半隐入位（位置+scale+圆角同时平滑过渡）。
3. **设置按钮切换逻辑改为"显示当前状态"**
   - 主题/语言/隐藏文件/显示行号 四个二态开关：按钮文案和图标显示**当前状态**。
   - 主题按钮加双图标（太阳/月亮）随状态切换；暗色时 .checked=true 显示月亮。
   - 当前暗色→"暗色"+月亮；当前中文→"中文"；当前隐藏中→"隐藏"+划眼；当前有行号→"行号"+有序列表。
4. **i18n 补全**
   - 浮窗标题、首页链接、气泡、加载/刷新/空仓库/分支菜单/删除弹窗 全套中英文。
   - 切换分支 toast、删除失败 toast、文件加载失败 toast 补全国际化。
   - viewer + 首页 applyLang 动态设置 `<html lang>` 属性。
5. **全面代码 review**
   - 检查了事件绑定重复、闭包变量、CSS 优先级、XSS 防护（escapeHtml/Go 模板转义）、touch 事件。
   - 无严重 bug。设计文档要求基本满足。
