...
| GET    | `/healthz`                 | 健康检查 |
| GET    | `/:pathid/*filepath`       | 拉取并渲染站点内文件，空路径回退 `index.html`，`?page=N` 翻页 |

**注册鉴权**：动态注册默认应受 `admin_token` 保护（`Authorization` 头校验），
否则公开部署会变成开放代理。详见安全章节。
...
- **HTML 原样输出的 XSS**：渲染的是用户注册仓库的任意 HTML。由于全部站点同源
  （同一 `BASE_URL` 下的不同 path），站点间**不再有子域名隔离**——一个站点的脚本
  理论上可访问同源的其它路径与首页/管理接口。缓解：
  - 管理 API 用 `Authorization` 头 + `admin_token`，**不使用 cookie**，避免被同源脚本窃取会话。
  - 站点页面响应设置严格 `Content-Security-Policy`，限制脚本来源；
    必要时对 `.html` 提供「沙箱模式」（`<iframe sandbox>` 承载或剥离脚本）。
  - 文档明确：开放给不可信仓库时，path 同源模型的隔离弱于子域名方案。
- **注册 API 滥用**：动态注册默认需 `admin_token`；公开部署务必开启。
- **SSRF / 内网探测**：用户传入 git_url 可能指向内网。Provider 解析出的目标 host
  需经白名单/黑名单校验，默认禁止解析到私网/环回地址（除非显式允许，便于自建内网 git）。
- **凭据保护**：私有凭据存内存、不落盘、不在 `List`/日志明文返回；配置用环境变量插值。
...
- **安全默认全开**：动态注册需 `admin_token`；SSRF 内网拦截默认开启（可加白名单放行自建内网 git）；
  管理 API 仅用 `Authorization` 头鉴权、不使用 cookie；站点页面下发严格 CSP，
  对不可信仓库的 `.html` 可启用沙箱承载。
